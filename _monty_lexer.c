...
_istoken (const char *str)
{
  ...
  if (!strcmp (strd, "PUSH") || !strcmp (strd, "PALL")
      || !strcmp (strd, "POP") || !strcmp (strd, "PINT")
      || !strcmp (strd, "SWAP") || !strcmp (strd, "ADD")
      || !strcmp (strd, "NOP"))
    {
...
void
_monty_lexer (char *buffer, const unsigned int file_line_number,
	      stack_t **start)
{
  ...
  while (token != NULL)
      ...
      if (_istoken (token) || push_flag == true)
	{
	  ...
	  strd = _string_toupper (strd);
	  if (((!strcmp (strd, "PUSH") && check_line_pos > 0)
	       || (push_flag == true && check_line_pos > 0)))
	    {
	      free (strd);
		/**
	      ...
	      if (push_flag == true)
		{
		  ...
		  push_cmd =
		    _realloc (push_cmd, sizeof (push_cmd),
			      (sizeof (push_cmd) + sizeof (token)));
